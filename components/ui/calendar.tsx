...
function Calendar({
  ...
  function hasSelected(p: any): p is { selected: unknown } {
    return 'selected' in p;
  }

  const selected = hasSelected(props) ? props.selected : undefined;

  // Helper to extract a Date from selected (handles array, Date, undefined)
  function getSelectedDate(sel: any): Date | undefined {
    if (sel instanceof Date) return sel;
    if (Array.isArray(sel) && sel[0] instanceof Date) return sel[0];
    return undefined;
  }
...
function YearGrid({
  ...
  const selectedDate: any = selected;
        ...
        return (
          <Button
            ...
            onClick={() => {
              ...
              } else if (Array.isArray(selected) && selectedDate[0] instanceof Date) {
